...
run_train = 1
if (run_train):
    csvfile = 'output/data27_overall.csv'
    ...
    rnn_model = RNN_module.rnn_train(train_X, train_Y, batch_size=32, epochs=3, window_size=10)
    RNN_module.plot_highest_error_period(rnn_model, test_X, test_Y, dataset, normalizer, window_size=10080)

run_main = 1
if (run_main):
    #TEST ON OVERALL CONSUMPTION FOR A WHOLE MONTH
    csvfile = 'output/data27_1month_overall.csv'
    overall_df = pd.read_csv(csvfile)
    overall_df = pd.DataFrame(overall_df.iloc[:, 2]) 
